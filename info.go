...
func (info *Info) FillFromJSON(r io.Reader) error {
	...
	info.Height = toUInt64(jsonParsed.Path("height").Data())
	info.Width = toUInt64(jsonParsed.Path("width").Data())
	info.ThumbnailHeight = toUInt64(jsonParsed.Path("thumbnail_height").Data())
	info.ThumbnailWidth = toUInt64(jsonParsed.Path("thumbnail_width").Data())
